...
struct PreferencesView: View {
        ...
        Section(languageService.s("other")) {
            ...
            if showNotifications {
                Button(languageService.s("test_notification")) {
                    NotificationService.shared.sendDownloadCompleted(filename: "Hyperscalar Test", languageService: languageService)
                }
                .buttonStyle(.link)
                .controlSize(.small)
            }
    ...
    private var aboutTab: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 16) {
                    Link(destination: URL(string: "https://github.com/offsideai/Hyperscalar")!) {
    ...
    private func updateYtdlp() {
        ...
        Task {
            ...
            let hyperscalarDir = appSupport.appendingPathComponent("Hyperscalar")
            let destination = hyperscalarDir.appendingPathComponent("yt-dlp")
            
            do {
                try FileManager.default.createDirectory(at: hyperscalarDir, withIntermediateDirectories: true)
